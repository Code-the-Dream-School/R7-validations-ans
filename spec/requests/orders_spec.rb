...
RSpec.describe "Orders", type: :request do
  ...
  describe "post orders_path with invalid data" do
    it "does not save a new entry or redirect" do
      order_attributes = FactoryBot.attributes_for(:order)  # customer_id will be invalid
      expect {
        post orders_path, params: {order: order_attributes}
  ...
  describe "put order_path with valid data" do
    it "updates an entry and redirects to the show path for the order" do
      order = FactoryBot.create(:order)
      put order_path(id: order.id), params: {order: {product_name: 'hinges'}}
      order.reload
      expect(order.product_name).to eq('hinges')
  ...
  describe "put order_path with invalid data" do
    it "does not update the order record or redirect" do
      order = FactoryBot.create(:order)
      put order_path(id: order.id), params: { order: { product_count: -2}}
      order.reload
      expect(order.product_count).to_not eq(-2)
      expect(response).to render_template(:edit)
    end
  end
